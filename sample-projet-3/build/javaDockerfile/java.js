...
app.post('/java/validate', (req, res) => {
    const { code, testCode, idUser } = req.body;
    ...
    exec('javac /tmp/Main.java', (syntaxErr, stdout, stderr) => {
        ...
        exec('java -cp /tmp Main', (execErr, execStdout, execStderr) => {
            ...
            exec(`javac -cp /tmp:${JUNIT_JAR} -d /tmp /tmp/MainTest.java && java -cp /tmp:${JUNIT_JAR}:${HAMCREST_JAR} org.junit.runner.JUnitCore MainTest`, (testErr, testStdout, testStderr) => {
                if (testErr) {
